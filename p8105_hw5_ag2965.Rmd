...
homicides_df_total <- read.csv(url("https://raw.githubusercontent.com/washingtonpost/data-homicides/master/homicide-data.csv"))%>%
  janitor::clean_names()%>%
  count()

homicides_df_state <- read.csv(url("https://raw.githubusercontent.com/washingtonpost/data-homicides/master/homicide-data.csv"))%>%
  janitor::clean_names()%>%
  group_by(state)%>%
  summarize

homicides_df_city <- read.csv(url("https://raw.githubusercontent.com/washingtonpost/data-homicides/master/homicide-data.csv"))%>%
  janitor::clean_names()%>%
  group_by(city)%>%
  summarize
```
...
```{r}
homicides_city_state <- read.csv(url("https://raw.githubusercontent.com/washingtonpost/data-homicides/master/homicide-data.csv"))%>%
  janitor::clean_names()%>%
    unite(city_state, c(city, state), sep = ", ")%>%
    group_by(city_state)%>%
  count()

homicides_unsolved <- read.csv(url("https://raw.githubusercontent.com/washingtonpost/data-homicides/master/homicide-data.csv"))%>%
  janitor::clean_names()%>%
    unite(city_state, c(city, state), sep = ", ")%>%
    group_by(city_state)%>%
...
```{r}
#All homicides in Baltimore, MD
homicides_baltimore <- read.csv(url("https://raw.githubusercontent.com/washingtonpost/data-homicides/master/homicide-data.csv"))%>%
  janitor::clean_names()%>%
    unite(city_state, c(city, state), sep = ", ")%>%
    filter(city_state=="Baltimore, MD")%>%
    count()
  
#All unsolved homicides in Baltimore, MD
homicides_baltimore_unsolved <- read.csv(url("https://raw.githubusercontent.com/washingtonpost/data-homicides/master/homicide-data.csv"))%>%
  janitor::clean_names()%>%
    unite(city_state, c(city, state), sep = ", ")%>%
    filter(city_state=="Baltimore, MD")%>%
    filter(disposition=="Closed by arrest" | disposition=="Open/No arrest")%>%
...
proptest=homicides_baltimore%>%
  prop.test(
    x="homicides_baltimore_unsolved",
    n="n_obs",
